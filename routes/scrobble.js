...
router.post('/transfer', async (req, res) => {
	...
	const session_key = db.prepare('SELECT session_key FROM users WHERE lastfm_username = ?').get(req.session.username);

	const fetched_tracks = await getAllRecentTracks(source_user, from, to);
	const formatted_tracks = fetched_tracks.map( track => ({
	...
	for (let i = 0; i < formatted_tracks.length; i+= 50) {
		result = await scrobbleTracks(session_key, formatted_tracks.slice(i, i+50));
	}
